...
function wireChildProcessLifecycle(
  ...
  function handleError(error) {
    ...
    log(
      `[meteoragent] ${failedToSpawn ? "could not run the Next.js process" : "Next.js process error"}: ${error.message}`,
    );
  ...
  child.once("exit", (code, signal) => {
    ...
    if (!shuttingDown) {
      log(
        `[meteoragent] Next.js exited unexpectedly (${signal ? `signal ${signal}` : `code ${code}`})`,
      );
    }
