...
if (!loopbackHostnames.has(hostname)) {
  if (passwordEnabled) {
    console.warn(
      `Warning: MeteorAgent is listening on ${hostname} with password authentication over HTTP. Use HTTPS or a trusted VPN to protect the password in transit.`,
    );
  } else {
    console.warn(
      `Warning: MeteorAgent is listening on ${hostname} without authentication. Only use this on a trusted network.`,
    );
  }
}
