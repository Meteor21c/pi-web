export async function terminalRequest(path: string, options?: RequestInit): Promise<{ id?: string; cwd?: string }> {
  ...
  const data = await response.json().catch(() => {
    throw new Error(`Terminal request failed (HTTP ${response.status}): server returned an empty or invalid JSON response. Check the MeteorAgent server log.`);
  });
  if (!data || typeof data !== "object") throw new Error(`Terminal request failed (HTTP ${response.status}): invalid JSON response. Check the MeteorAgent server log.`);
