...
test("bounds the server-side plugin search cache", async () => {
  const originalFetch = globalThis.fetch;
  ...
  globalThis.fetch = async () => new Response(card({ name: "new-result", types: "prompt" }), { status: 200 });
  try {
    await getCommunityPluginCatalog(false, "new query");
    assert.equal(globalThis.__magentPluginCatalogSearchCache.size, 100);
    assert.equal(globalThis.__magentPluginCatalogSearchCache.has("old-0"), false);
    assert.equal(globalThis.__magentPluginCatalogSearchCache.has("new query"), true);
  } finally {
    globalThis.fetch = originalFetch;
    globalThis.__magentPluginCatalogSearchCache = undefined;
  }
