...
test("drops a truncated event-only frame instead of exposing empty JSON to the OpenAI SDK", async () => {
  const frames = [
    event(JSON.stringify({ type: "response.created", response: { id: "resp_sdk_2", status: "in_progress" } })),
    "event: response.o:\n\n",
    "event: response.output_item.added\n" + event(JSON.stringify({
      type: "response.output_item.added",
      output_index: 0,
      item: { id: "msg_sdk_2", type: "message", role: "assistant", content: [] },
    })),
    ...
    event(JSON.stringify({
      type: "response.completed",
      response: { id: "resp_sdk_2", status: "completed", output: [] },
    })),
    event("[DONE]"),
  ];
  const client = new OpenAI({
...
test("drops comment-only relay heartbeats", async () => {
  const payload = JSON.stringify({ type: "response.created", response: { id: "resp_heartbeat", status: "in_progress" } });
  const fetch = createRelayResponsesFetch(async () => responseFor([": keepalive\n\n", event(payload)]));
  const response = await fetch("https://relay.example.test/v1/responses");
  assert.equal(await response.text(), event(payload));
});
