...
const { preserveRelayProtocolSelections, sessionAffinityCompat } = await jiti.import("./relay-config-save.ts");
...
test("relay protocols persist session-affinity headers for exact billing reconciliation", () => {
  assert.deepEqual(sessionAffinityCompat("openai-completions"), {
    sendSessionAffinityHeaders: true,
    sessionAffinityFormat: "openai",
  });
  assert.deepEqual(sessionAffinityCompat("anthropic-messages"), {
    sendSessionAffinityHeaders: true,
  });
  assert.equal(sessionAffinityCompat("openai-responses"), undefined);
});

test("catalog sync preserves only the user's OpenAI fallback protocol choice", () => {
  const refreshed = preserveRelayProtocolSelections([
    { id: "gpt-5.6-sol", api: "openai-responses", contextWindow: 258_000 },
    { id: "claude-sonnet-4-6", api: "anthropic-messages" },
  ], {
    models: [
      { id: "gpt-5.6-sol", api: "openai-completions", contextWindow: 10 },
      { id: "claude-sonnet-4-6", api: "openai-completions" },
    ],
  });

  assert.equal(refreshed[0].api, "openai-completions");
  assert.equal(refreshed[0].contextWindow, 258_000);
  assert.equal(refreshed[1].api, "anthropic-messages");
});
...
test("sync preserves double-k identity, counts saved mixed models, and never rewrites tokens", async () => {
  ...
  const savedMixedModels = disk("models.json").providers["meteor21c-k-k123"].models;
  assert.equal(savedMixedModels.length, 2);
  assert.deepEqual(savedMixedModels.find((model) => model.id === "claude-sonnet-4-6").compat, {
    sendSessionAffinityHeaders: true,
  });
  assert.equal(savedMixedModels.find((model) => model.id === "gpt-5.6").compat, undefined);
...
test("sync stores safe nested group metadata without persisting the full key", async () => {
  ...
  assert.deepEqual(disk("models.json").providers[result.providers[0].providerId].models[0].compat, {
    sendSessionAffinityHeaders: true,
    sessionAffinityFormat: "openai",
  });
});
...
test("complete empty key list removes synced and unscoped legacy providers but keeps unrelated custom providers", async () => {
  writeFileSync(join(process.env.PI_CODING_AGENT_DIR, "models.json"), JSON.stringify({ providers: {
    "meteor21c-k123": { models: [] },
    meteor21c: { models: [] },
    "meteor21c-claude": { models: [] },
    "meteor21c-openai": { models: [] },
  ...
  assert.equal(disk("models.json").providers.meteor21c, undefined);
  assert.equal(disk("models.json").providers["meteor21c-claude"], undefined);
  assert.equal(disk("models.json").providers["meteor21c-openai"], undefined);
  assert.ok(disk("models.json").providers["custom-provider"]);
});
...
test("a successful catalog with no chat models clears the old authorization snapshot", async () => {
  const accountId = groupStore.stableRelayAccountId("mock@example.test");
  const providerId = "meteor21c-k9";
  writeFileSync(join(process.env.PI_CODING_AGENT_DIR, "models.json"), JSON.stringify({ providers: {
    [providerId]: { models: [{ id: "gpt-5.6" }] },
  } }));
  groupStore.writeRelayGroups([{
    accountId,
    providerId,
    keyId: "9",
    keyName: "old",
    maskedKey: "sk-old...0009",
    modelIds: ["gpt-5.6"],
    syncedAt: 1,
  }]);
  globalThis.fetch = async (url) => url.includes("/keys?")
    ? response({ data: { items: [{ id: 9, key: "fake-9" }] } })
    : url.includes("/model-plaza")
      ? response({ data: { groups: [] } })
      : response({ data: [{ id: "text-embedding-3-small" }] });

  const result = await syncRelayProviders();
  assert.equal(result.reason, "no-usable-key");
  assert.deepEqual(result.warnings, [{ providerId, reason: "empty-catalog" }]);
  assert.equal(disk("models.json").providers[providerId], undefined);
  assert.deepEqual(groupStore.readRelayGroups()[0].modelIds, []);
});
