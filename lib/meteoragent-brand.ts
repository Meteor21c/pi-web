...
export function rebrandMeteorAgentSystemPrompt(prompt: string): string {
  if (!prompt.includes("You are an expert coding assistant operating inside pi, a coding agent harness.")) {
    return prompt;
  }
  return prompt
    .replace(
    ...
    .replace(/Inspect PI_\* environment variables for current model and session details\.?/g, "Inspect the runtime state for current model and session details.")
