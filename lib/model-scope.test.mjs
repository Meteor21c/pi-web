...
const { filterModelScopeByProviders, filterModelScopeByRelayAuthorization, resolveVisibleModels, selectInitialModelScope } = await loadSubject();
...
test("product provider filtering keeps only account-indexed groups and their thinking pins", async () => {
  const scope = await resolveVisibleModels(runtime, [
    "anthropic/claude-opus-5:high",
    "acme-gateway/*:low",
    "acme-gateway-openai/*:medium",
  ]);
  const filtered = filterModelScopeByProviders(scope, new Set(["acme-gateway"]));

  assert.deepEqual(refs(filtered).sort(), [
    "acme-gateway/claude-opus-4-8",
    "acme-gateway/claude-sonnet-5",
  ]);
  assert.ok(filtered.scopedModels.every((entry) => entry.model.provider === "acme-gateway"));
  assert.deepEqual(Object.keys(filtered.thinkingLevelPins).sort(), [
    "acme-gateway/claude-opus-4-8",
    "acme-gateway/claude-sonnet-5",
  ]);
  assert.equal(filterModelScopeByProviders(scope, null), scope);
});

test("relay authorization filtering removes models outside each API key snapshot", async () => {
  const scope = await resolveVisibleModels(runtime, undefined);
  const filtered = filterModelScopeByRelayAuthorization(scope, new Map([
    ["acme-gateway", new Set(["claude-opus-4-8"])],
    ["acme-gateway-openai", new Set()],
  ]));

  assert.deepEqual(refs(filtered), [
    "anthropic/claude-opus-5",
    "anthropic/claude-sonnet-5",
    "anthropic/claude-sonnet-4-6",
    "acme-gateway/claude-opus-4-8",
  ]);
  assert.equal(filtered.scopedModels.length, 0);
  assert.deepEqual(filtered.thinkingLevelPins, {});
});
