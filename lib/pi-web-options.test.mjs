...
test("supports --help and -h without starting the server", () => {
  ...
  assert.match(getHelpText(), /Usage: webagent/);
...
test("supports --version and -v without starting the server", () => {
  ...
  const full = spawnSync(process.execPath, [cliPath, "--version"], { encoding: "utf8" });
  assert.equal(full.status, 0);
  assert.equal(full.stdout.trim(), "0.1.4");

  const short = spawnSync(process.execPath, [cliPath, "-v"], { encoding: "utf8" });
  assert.equal(short.status, 0);
  assert.equal(short.stdout.trim(), "0.1.4");
});
...
test("CLI writes help and parse errors before exiting", () => {
  ...
  assert.match(help.stdout, /Usage: webagent/);
